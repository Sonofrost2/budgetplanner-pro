...
name: Public demo account
 description: Shared demo@budgetplanner-pro.eurekaci.dev user, daily pg_cron reset, /demo route, DemoBanner with upgrade CTA
type: feature
---
 Compte démo public partagé : `demo@budgetplanner-pro.eurekaci.dev` / `DemoBudget2026!` (créé via auth.users dans la migration). Plan Gratuit, onboarding bypass dans DashboardLayout via `isDemoUserEmail`.
